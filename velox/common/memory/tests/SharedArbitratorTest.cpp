...
// 30-second timeout below. The arbitrator also runs
// memoryArbitrationStateCheck which directly asserts the requesting
// driver is suspended; without the wrap the test fails with
...
// memoryArbitrationStateCheck passes; reclaim's requestPause().wait()
// finds the driver already off-thread; the allocation completes; the
// task makes forward progress.
TEST_P(
    SharedArbitrationTestWithThreadingModes,
