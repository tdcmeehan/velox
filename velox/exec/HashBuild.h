...
class HashBuild final : public Operator {
 ...
 private:
  ...
  // finishHashBuild() before prepareJoinTable() so the callback may inspect
  // per-driver VectorHasher state before the merge clears it, and may
  // allocate from arbitrator-tracked task-child pools safely. A
  // RAII guard ensures the driver is resumed on every exit path including
  // the firing-site swallowing a callback exception.
